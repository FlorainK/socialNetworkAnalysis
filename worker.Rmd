...
genreCount <- nodes %>%
  filter(spotify_id %in% biggestArtists$spotify_id) %>%
  mutate(genres = strsplit(genres, ",")) %>%
  unnest(genres) %>%
  count(genres)

# get the count of genres of the biggest german artists
genreCountGerman <- nodes %>%
  filter(spotify_id %in% biggestGermanArtists$spotify_id) %>%
  mutate(genres = strsplit(genres, ",")) %>%
  unnest(genres) %>%
  count(genres)
```
...
# plot the genres with the most followers and safe it as a png
barplot(genresFollowers$followers, names.arg = genresFollowers$genre, las = 2, cex.names = 0.5, col = "blue")
dev.copy(png, file = "genresFollowers.png")
dev.off()
