...
```{r}
#  get all german artists
germanArtistsNodes <- nodes[nodes$genre %like% "german" | nodes$genre %like% "deutsch", ]

# filter all edges if they are german artists
# -> Nur einer der beiden muss deutsch sein
germanArtistsEdges <- subset(edges, id_0 %in% germanArtistsNodes$spotify_id | id_1 %in% germanArtistsNodes$spotify_id)

# add non german artits from the edges to the germanArtistsNodes
germanArtistsNodes <- rbind(germanArtistsNodes, nodes[match(germanArtistsEdges$id_0, nodes$spotify_id), ])
germanArtistsNodes <- rbind(germanArtistsNodes, nodes[match(germanArtistsEdges$id_1, nodes$spotify_id), ])


# rename id_0 to source and id_1 to target
colnames(germanArtistsEdges) <- c("source", "target")

# replace the id in germanArtists by a number from 1 to n
germanArtistsNodes$spotify_id <- as.numeric(factor(germanArtistsNodes$spotify_id))


#  replace each id in biggestArtistsCollaborations with the new_id
germanArtistsEdges$source <- as.numeric(factor(germanArtistsEdges$source))
germanArtistsEdges$target <- as.numeric(factor(germanArtistsEdges$target))


# order the nodes by spotify_id
germanArtistsNodes <- germanArtistsNodes[order(germanArtistsNodes$spotify_id), ]

# drop duplicates
germanArtistsNodes <- germanArtistsNodes[!duplicated(germanArtistsNodes$spotify_id), ]
# set spotify_id as rownames
rownames(germanArtistsNodes) <- germanArtistsNodes$spotify_id

# add column with true or false if artist in germanArtistsNodes
germanArtistsNodes$isGerman <- germanArtistsNodes$spotify_id %in% germanArtistsNodes$spotify_id


# drop spotify_id column
germanArtistsNodes <- germanArtistsNodes[, -1]

# change the wording of isGerman to "TOP 5" or "OTHER"
germanArtistsNodes$isGerman <- ifelse(germanArtistsNodes$isGerman == TRUE, "TOP 5", "OTHER")
...
d <- forceNetwork(
  ...
  fontSize = 20,
  Nodesize = "followers",
  radiusCalculation = JS("(d.nodesize / 1000000)+5"),
  zoom = TRUE,
  colourScale = JS("d3.scaleOrdinal([ '#05f2fa','#021433'])"),
  legend = TRUE,
)

saveNetwork(d, file = "germanMusicScene.html")
```
