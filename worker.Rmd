...
library(networkD3)
# get the 20 biggest artists
biggestArtists <- nodes[order(nodes$followers, decreasing = TRUE), ][1:5, ]

# get the collaborations of the 20 biggest artists
biggestArtistsCollaborations <- subset(filteredEdges, id_0 %in% biggestArtists$spotify_id | id_1 %in% biggestArtists$spotify_id)

# replace the spotify ids with the artist names
biggestArtistsCollaborations$id_0 <- nodes[match(biggestArtistsCollaborations$id_0, nodes$spotify_id), "name"]
biggestArtistsCollaborations$id_1 <- nodes[match(biggestArtistsCollaborations$id_1, nodes$spotify_id), "name"]


p <- simpleNetwork(biggestArtistsCollaborations)
